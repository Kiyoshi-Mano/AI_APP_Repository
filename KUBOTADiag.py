...
def main():
    ...
    .sidebar .stDateInput label, .sidebar .stCheckbox label,
    .sidebar .stSlider label, .sidebar .stRadio label {
        color: #ecf0f1 !important;
        font-weight: 500;
    }
    ...
    /* Input styling */
    .stSelectbox select, .stMultiSelect select, .stTextInput input {
        border-radius: 10px;
        border: 2px solid #e9ecef;
        transition: all 0.3s ease;
    }

    .stSelectbox select:focus, .stMultiSelect select:focus, .stTextInput input:focus {
        border-color: #667eea;
    ...
    <div style="text-align: center; margin-bottom: 2rem;">
        <h1 style="margin-bottom: 0.5rem;">🔧 {APP_TITLE}</h1>
        <p style="color: #6c757d; font-size: 1.2rem; font-weight: 400;">
            高度な分析とインサイトによる建機修理データの可視化プラットフォーム
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        ">
            <h2 style="color: white; margin: 0; font-size: 1.5rem; font-weight: 600;">
                ⚙️ 設定パネル
            </h2>
    ...
    with st.sidebar:
        ...
        ">
            ...
            <span style="font-size: 1.2rem; font-weight: bold;">{len(df_raw):,}行</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        # Categorical filters with enhanced styling
        categorical_fields = {
            '機種名': '🚜',
            'シリーズ': '📋',
            '種別': '🏷️',
        ...
        for field, icon in categorical_fields.items():
            if field in df_normalized.columns:
                ...
                if len(unique_values) > 0:
                    with st.expander(f"{icon} {field} ({len(unique_values)}種類)"):
        ...
        if 'アワ帯' in df_normalized.columns:
            ...
            st.markdown("#### ⏱️ アワメータ")
            selected_hour_bins = st.multiselect(
                "アワメータ帯域を選択",
                hour_bins,
                help="アワメータ帯域でフィルタ"
            )
    ...
    with st.sidebar:
        ...
        # Analysis options
        st.markdown("#### 📊 分析設定")
        filters['exclude_inspections'] = st.checkbox(
            "点検整備を除外",
            help="点検・検査項目を集計から除外"
        )

        filters['aggregation_unit'] = st.radio(
            "集計単位",
            ["cases", "parts"],
    ...
    with st.sidebar:
        ...
        ">
            <strong>📊 フィルタ結果</strong><br>
            <span style="font-size: 1.4rem; font-weight: bold;">{len(df_cases_filtered):,}</span>
            <span style="font-size: 0.9rem;">案件</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if filters.get('use_sampling', False) and filters.get('sample_size'):
            ...
            if len(df_cases_filtered) < original_count:
                ...
                ">
                    ⚡ 高速表示モード<br>
                    {filters['sample_size']:,}行でサンプリング表示中
                </div>
                """, unsafe_allow_html=True)
